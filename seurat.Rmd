---
title: "Seurat workflow on LiLi female cells smart-seq data"
output:
  html_document:
    keep_md: true
    smart: false
...
cellIndex <- metaDataFemale$Transcripts>100000 & 
             ...
             metaDataFemale$Gene_Numer<10000
...
### Reverse the TPM calculation
* Use the per cell total transcripts from the metadata table
* For each cell, multiply the normalized counts by the total number of transcripts for that cell. Then divide by 1000000 
...
transcripts <- t(metaDataFemale[-c(1:4)])
#names(trancripts) <- rownames(metaDataFemale )
transcripts <- transcripts[,colnames(exprTableAfterQC)] 
transcriptByMil <- transcripts/1000000
exprTableRaw <- data.frame(mapply(`*`,exprTableAfterQC, as.vector(transcriptByMil)))
row.names(exprTableRaw) <- rownames(exprTableAfterQC)
...
FGC <- CreateSeuratObject(raw.data = exprTableRaw,
                          min.cells = 5)
...
Data <- as.data.frame(as.matrix(FGC@data))
cat("All genes:\n")
print(unname(genes))
...
totalSum <- Matrix::colSums(FGC@data)
...
percentages <- FGC@meta.data$percent.dissoc
hist(percentages, 
     breaks = 100, 
     col = "lightgrey", 
...
```{r, message=FALSE}
FGC <- FindVariableGenes(object = FGC, 
                         mean.function = ExpMean, 
                         dispersion.function = LogVMR,
                         x.low.cutoff = 0.0125, 
                         x.high.cutoff = 8, 
                         y.cutoff = 1)
length(x = FGC@var.genes)
topAveExpr = FGC@hvg.info[FGC@hvg.info[,1]>3, ]
...
```{r, message=FALSE}
FGC <- RunPCA(object = FGC, pc.genes = FGC@var.genes, do.print = TRUE, pcs.print = 1:5, 
    genes.print = 5)
```

```{r, message=FALSE}
VizPCA(object = FGC, pcs.use = 1:2)
ggsave(paste0(outputDir, "vizPCA.pdf"), width = 10, height = 7)
PCAPlot(object = FGC, dim.1 = 1, dim.2 = 2)
FGC <- ProjectPCA(object = FGC, do.print = FALSE)
write.table(FGC@dr$pca@gene.loadings, file = paste0(outputDir, "/geneLoadings.tsv"), sep = "\t", quote = FALSE)
```

```{r, message=FALSE}
PCHeatmap(object = FGC, pc.use = 1, cells.use = 500, do.balanced = TRUE, label.columns = FALSE)
```

```{r, message=FALSE}
PCHeatmap(object = FGC, pc.use = 1:12, cells.use = 500, do.balanced = TRUE, 
    label.columns = FALSE, use.full = FALSE)
```

```{r, message=FALSE}
#JackStrawPlot(object = FGC, PCs = 1:12)
```

```{r, message=FALSE}
PCElbowPlot(object = FGC)
...
totalSdev <- sum(FGC@dr$pca@sdev)
print(FGC@dr$pca@sdev/totalSdev)
```

```{r, message=FALSE}
FGC <- FindClusters(object = FGC, reduction.type = "pca", dims.use = 1:13, 
    resolution = 0.6, print.output = 0, save.SNN = TRUE, force.recalc = TRUE)
```

```{r, message=FALSE}
FGC <- RunTSNE(object = FGC, dims.use = 1:13, do.fast = TRUE)
FGC <- RunUMAP(FGC, reduction.use = "pca", dims.use = 1:13)
```

```{r, message=FALSE}
p <- TSNEPlot(object = FGC, do.return = TRUE)
ggplotly(p, width = 700, height = 500)
ggsave(paste0(outputDir, "/tSNE.pdf"), width = 10, height = 7)
p <- DimPlot(FGC, reduction.use = "umap", do.return = TRUE)
ggplotly(p, width = 700, height = 500)
ggsave(paste0(outputDir, "/UMAP.pdf"), width = 10, height = 7)
p <- PCAPlot(object = FGC, dim.1 = 1, dim.2 = 2, do.return = TRUE) 
ggplotly(p, width = 700, height = 500)
...
list <- (strsplit( colnames(FGC@data), split = "_"))
weeks <- sapply(list, function(el){el[2]} )
FGC@meta.data$embryo.age <- weeks
p <- TSNEPlot(object = FGC,
              group.by = "embryo.age",
              do.return = TRUE)
ggplotly(p, width = 700, height = 600)
